...
gpuError_t BatchQKApplyRotaryPosIdsCosSinCache(
    ...
    size_t k_rope_stride_h, bool interleave, gpuStream_t stream = nullptr) {
  int dev_id = 0;
  int num_sms = 0;
  ...
  DISPATCH_INTERLEAVE(interleave, INTERLEAVE, {
    DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
      ...
      if ((nnz + bdy - 1) / bdy >= num_ctas_0) {
        dim3 nblks(nblks_x);
        dim3 nthrs(bdx, bdy);
        FLASHINFER_CUDA_CALL(gpuLaunchKernel((void*)kernel_0, nblks, nthrs, args, 0, stream));
      } else {
        ...
        auto kernel_1 =
            BatchQKApplyRotaryPosIdsCosSinCacheHeadParallelismKernel<INTERLEAVE, HEAD_DIM, vec_size,
                                                                     bdx, DType, IdType>;
        FLASHINFER_CUDA_CALL(gpuLaunchKernel((void*)kernel_1, nblks, nthrs, args, 0, stream));
      }
    });
  });
